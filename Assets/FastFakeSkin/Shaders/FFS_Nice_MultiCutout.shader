...
Shader "Skin/FFSFull_Cutout_MultiMask" {
        ...
        Properties{
            ...
            _MaskVec("Mask Channels", vector) = (0, 0, 0, 1)
        }
        ...
        SubShader{
            ...
            void surf(Input IN, inout SurfaceOutputStandardSkin o) {
                ...
                half4 cutout = tex2D(_CutoutTex, IN.uv_MainTex);
                o.Alpha = 1.0 + ((cutout.x - 1.0) * _MaskVec.x + (cutout.y - 1.0) * _MaskVec.y + (cutout.z - 1.0) * _MaskVec.z + (cutout.w - 1.0) * _MaskVec.w);
            }
            ENDCG
        }
        FallBack "Diffuse"
